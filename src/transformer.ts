...
export function transform(
    ...
    const members = stack.map((node) => {
      switch (node.kind) {
        ...
        case ts.SyntaxKind.ObjectLiteralExpression: {
          if (node.interfaceCandidate) {
            const item = namedProp({ name: node.name });
            item.type = ts.createTypeReferenceNode(
              newInterfaceName(node),
              undefined
            );
            return item;
          } else {
  ...
  function getArrayElementsType(node: ParsedNode): any {
    ...
    if (kinds.size === 1) {
      // if there's only 1 kind in the array, it's safe to use type[];
      ...
      switch (kind) {
        ...
        case ts.SyntaxKind.ObjectLiteralExpression:
          const item = ts.createTypeReferenceNode(
            getArrayInterfaceItemName(node.name),
            undefined
          );
          return item;
        default:
      ...
      // a mix of true/false is still a boolean[];
      if (
    ...
    if (kinds.every((kind) => safeUnions.has(kind))) {
      ...
      (item as any).type = ts.createUnionTypeNode(types);
      return item;
    } else {
